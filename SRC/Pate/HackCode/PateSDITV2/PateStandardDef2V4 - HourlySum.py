...
class PATEAnomalyDetector:
    def __init__(self, e_buffer=100, d_buffer=100, binary_scores=False, window_size=100):
        ...
        self.window_size = window_size  # The rolling window size (100 hours)
    ...
    def preprocess_data(self, df):
        """
        Preprocesses the data to sum the values for each hour.
    ...
    def add_data(self, timestamp, value):
        ...
        # Ensure only the last `window_size` hours of data are kept
        if len(self.time_series_data) > self.window_size:
...
if __name__ == "__main__":
    ...
    detector = PATEAnomalyDetector(e_buffer=100, d_buffer=100, binary_scores=False, window_size=100)
    ...
    # Add data points to the detector, processing in a rolling window of 100 hours
