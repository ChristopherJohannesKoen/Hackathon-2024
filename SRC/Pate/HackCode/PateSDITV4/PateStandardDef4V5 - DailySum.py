...
class SyntheticLabelPATEAnomalyDetector:
    ...
    def preprocess_data(self, df):
        ...
        # Select only numeric columns (e.g., 'cost') for resampling
        numeric_columns = df.select_dtypes(include=[np.number]).columns
        df_resampled = df[numeric_columns].resample('D').sum()

        # Re-attach non-numeric columns (like usage_unit and service_type)
        df_non_numeric = df[['usage_unit', 'service_type']].resample('D').first()
...
if __name__ == "__main__":
    ...
    print("Preprocessing data to calculate daily sums...")
    
    # Preprocess data to calculate daily sums
    ...
    processed_data_path = os.path.join(output_dir, 'pateStandardDef4V5DailySumProcessedData.csv')
    ...
    attributes_path = os.path.join(output_dir, 'pateStandardDef4V5DailySumAttributes.txt')
