...
class PATEAnomalyDetector:
    def __init__(self, e_buffer=100, d_buffer=100, binary_scores=False, early_detection_threshold=0.5):
        self.e_buffer = e_buffer
        self.d_buffer = d_buffer
        self.binary_scores = binary_scores
        self.early_detection_threshold = early_detection_threshold
    ...
    def early_detection(self):
        """
        Applies an early detection strategy based on the time series data.

        :return: A list of potential early detected anomalies.
        """
        if not self.time_series_data:
            raise ValueError("No data available for early detection.")
        
        # Convert deque to numpy array for processing
        timestamps, values = zip(*self.time_series_data)
        y_score = np.array(values)
        
        # Early detection based on a rising trend or predefined threshold
        early_anomalies = [(timestamp, value) for (timestamp, value), score in zip(self.time_series_data, y_score) if score > self.early_detection_threshold]
        return early_anomalies
    
    def visualize_anomalies(self, anomalies, early_anomalies=None):
        """
        Visualizes the time series data and highlights the detected anomalies.

        :param anomalies: A list of detected anomalies.
        :param early_anomalies: A list of early detected anomalies (optional).
        """
        if not self.time_series_data:
        ...
        if early_anomalies:
            early_timestamps, early_values = zip(*early_anomalies)
            plt.scatter(early_timestamps, early_values, color='orange', label='Early Detected Anomalies')

        plt.title('Time Series Data with Detected Anomalies and Early Detection')
        plt.xlabel('Time')
        plt.ylabel('Value')
        plt.legend()
        plt.grid(True)
...
if __name__ == "__main__":
    ...
    detector = PATEAnomalyDetector(e_buffer=100, d_buffer=100, binary_scores=False, early_detection_threshold=120)
    ...
    for i in range(1000):
        ...
        if i % 100 == 0 and i > 0:
            ...
            early_anomalies = detector.early_detection()
            if anomalies or early_anomalies:
                print(f"Anomalies detected: {anomalies}")
                print(f"Early Anomalies detected: {early_anomalies}")
                detector.visualize_anomalies(anomalies, early_anomalies)
            else:
